...
namespace Renderer::Culling::Visibility
{
    Pipeline::Pipeline(const Vk::Context& context)
    {
        std::tie(handle, layout, bindPoint) = Vk::Builders::PipelineBuilder(context)
            ...
            .AddPushConstant(VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(Visibility::PushConstant))
            .Build();

        Vk::SetDebugName(context.device, handle, "VisibilityPipeline");
        Vk::SetDebugName(context.device, layout, "VisibilityPipelineLayout");
    }
}