...
namespace Renderer::AO::XeGTAO::DepthPreFilter
{
    ...
    )
    {
        std::tie(handle, layout, bindPoint) = Vk::Builders::PipelineBuilder(context)
            ...
            .AttachShader("AO/XeGTAO/DepthPreFilter.comp", VK_SHADER_STAGE_COMPUTE_BIT)
        ...
        Vk::SetDebugName(context.device, handle,                                         "DepthPreFilterPipeline");
        Vk::SetDebugName(context.device, layout,                                         "DepthPreFilterPipelineLayout");
        Vk::SetDebugName(context.device, textureManager.GetSampler(samplerIndex).handle, "DepthPreFilterPipeline/DepthSamplerIndex");
    }
}