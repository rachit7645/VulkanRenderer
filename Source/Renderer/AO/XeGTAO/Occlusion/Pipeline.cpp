...
namespace Renderer::AO::XeGTAO::Occlusion
{
    ...
    )
    {
        std::tie(handle, layout, bindPoint) = Vk::Builders::PipelineBuilder(context)
            ...
            .AttachShader("AO/XeGTAO/XeGTAO.comp", VK_SHADER_STAGE_COMPUTE_BIT)
        ...
        Vk::SetDebugName(context.device, handle,                                         "XeGTAOPipeline");
        Vk::SetDebugName(context.device, layout,                                         "XeGTAOPipelineLayout");
        Vk::SetDebugName(context.device, textureManager.GetSampler(samplerIndex).handle, "XeGTAOPipeline/SamplerIndex");
    }
}